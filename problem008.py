...
def largestProductinaSeries(series, n):
    if n < 0: return -1
    largest = 0
    product = 0
    start = 0
    while start+n <= len(series):
        if series[start] == 0:
            start += 1
            product = 0
            continue
        if product != 0:
            product = series[start+n-1]*product/series[start-1]
        else:
            product = 1
            for i in range(n):
                if series[start+i] == 0:
...
series = [7,3,1,6,7,1,7,6,5,3,1,3,3,0,6,2,4,9,1,9,2,2,5,1,1,9,6,7,4,4,2,6,5,7,4,7,4,2,3,5,5,3,4,9,1,9,4,9,3,
          ...
          4,1,4,5,3,5,1,0,0,4,7,4,8,2,1,6,6,3,7,0,4,8,4,4,0,3,1,9,9,8,9,0,0,0,8,8,9,5,2,4,3,4,5,0,6,5,8,5,4,
...
print(largestProductinaSeries(series, 13))
